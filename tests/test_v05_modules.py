...
from allspark.models import Resource, ResourceType, Goal
...
class TestEnvironmentAssessor:
    ...
    def test_assess_with_low_resources(self, db):
        ...
        assert any("电力" in f for f in result["threats"]["factors"])
    ...
    def test_assess_with_water_shortage(self, db):
        ...
        env = EnvironmentAssessor(db=db)
        result = env.assess()
        assert any("水源" in f for f in result["threats"]["factors"])
