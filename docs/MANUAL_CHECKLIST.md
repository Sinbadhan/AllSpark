...
| 1.18 | Executions navigation truth | in zh and en, open Executions from both desktop and mobile navigation; inspect the Dashboard and current Executions page | no duplicate Execute Task/启动操作 command is present; both viewports use the same Executions/执行中心 link; cross-page navigation reaches `/executions`; both current-page links expose `aria-current=page` |
...
| 1.20 | Knowledge advice to task | open one knowledge detail and choose Add to tasks twice; repeat with `task add <title>` in CLI | exactly one traceable knowledge task is created for the same entry; the manual CLI task is visible in Executions; source and Phase known/unknown truth are preserved |
