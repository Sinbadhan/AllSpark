...
| 1.3 | i18n round-trip | start zh → switch to en in CLI and Web → switch back | user-visible labels/errors follow the selected language; technical IDs, protocol names and user-authored content may retain their original form |
...
| 1.8W | Windows screen reader (Testing) | run the same core Web flow with NVDA on Windows when that environment is available | record pass/fail evidence; until a real run passes, Windows + NVDA remains Testing and must not be included in the Stable accessibility claim |
...
| 1.13 | Experimental crisis-support truth | in both languages, submit a clearly attributed quotation, a bare first-person quotation, a negated phrase, a direct first-person phrase, then confirm immediate danger; repeat with and without local `[crisis_support]` contacts | attributed quotation and clear negation do not false-trigger; a bare first-person quotation and direct disclosure ask a plain safety question before rule/LLM output; immediate danger prioritizes company, safer means access and local help; configured region/contact data is shown as user-provided and unverified; no diagnosis, authority-notified claim or silent diary/timeline/voice-history record appears |
| 1.13L | Web first-run language flow | open a fresh database once with a zh browser locale and once with en; switch zh → en → zh while moving forward/back and entering questionnaire values; repeat after L3 reset | language is the first actionable step and follows browser locale; every heading, option, placeholder and error rerenders; entered values survive both switches; every select and skip control has a stable accessible name |
| 1.14 | Mobile Dashboard core state | at 320/360/390/430px inspect zh and en with unconfigured, critical, sustained and long-unit resource data | no main-canvas horizontal scroll; title, phase, mode, every resource type/value/status and edit target remain fully visible without zoom or horizontal drag |
| 1.15 | CLI first-run language choice | launch a fresh database under zh, en, unavailable and one unsupported locale; make one invalid choice, then select each language | every pre-choice welcome/title/prompt/error is understandable in the locale-derived display language; unknown locale falls back to English; both choices self-identify in zh/en; locale default is not persisted until confirmation |
| 1.16 | Capability truth and Docker fallback | run CLI initialization and inspect System with (a) no Docker daemon/model/optional dependencies and (b) a fully configured test runtime; repeat System at 390px | hardware eligibility, dependency, configuration, running and EXP state remain distinct; only verified services say Running; Docker target and actual mode do not conflict; the module table is contained and horizontally scrollable on narrow screens |
| 1.17 | Repository responsive and keyboard flow | inspect long zh/en knowledge titles and IDs at 320/390/768px and desktop; search, paginate, open an item, exercise an empty result, then traverse the file tree with keyboard only | mobile shows an information-first list with complete title/verification/category/tier metadata and secondary ID; tablet/desktop retain the table; no page-level or core-list horizontal scroll; file-tree focus is visible and Enter/Space leave exactly one current item |
| 1.18 | Actions navigation truth | in zh and en, open Actions/行动 from both desktop and mobile navigation; inspect the Dashboard and current Actions page | no duplicate Execute Task/启动操作 command is present; both viewports use the same Actions/行动 label; cross-page navigation reaches the retained technical route `/executions`; both current-page links expose `aria-current=page` |
| 1.19 | Confirmed resource conversation | in Web and CLI, enter “two days of drinking water”, provide 10 L and 2 people, then test cancel and confirm in separate sessions | no resource, plan or timeline write occurs before confirmation or after cancel; confirmation records 10 L, derives only the user-supported 5 L/day group estimate, and refreshes the 24-hour plan; browser tabs do not share drafts |
| 1.20 | Knowledge dual-review gate and task idempotency | first open an unreviewed bundled entry and attempt task conversion through the API; then use the isolated, hash-matched `_fully_reviewed_entry()` fixture in `tests/test_sha241_knowledge_risk.py`, convert twice; separately use `task add <title>` in CLI | unreviewed content is withheld and conversion explicitly refused with zero task/plan writes; only the double-reviewed fixture creates exactly one traceable task; manual CLI tasks appear in Actions; source/Phase truth is preserved. Synthetic fixture approval is test-only, never professional acceptance or production metadata |
| 1.21 | Task result and reassessment | track the current primary action, then complete, fail and skip separate tasks with result/evidence; on one completion propose a new resource total but first leave confirmation unchecked | terminal status requires a non-empty result; evidence and completion time persist; the unconfirmed resource proposal changes nothing; confirmed total updates the resource, produces one reassessed active plan and exposes the next task |
| 1.22 | Dashboard truth and execution accessibility | with zero, partial and complete current resource facts, inspect Dashboard/footer/Executions at 320x568 and 1280x720; traverse result modal by keyboard and VoiceOver | resource count includes configured snapshots only; Phase/mode remain pending without evidence; no cross-resource 168h percentage bar appears; primary action and empty-state CTA remain discoverable; dialog traps focus, closes with Escape and announces errors |
