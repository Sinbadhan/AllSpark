...
| Experimental | 心理追踪与非临床危机支持（等待独立合格专家审阅）；治理/RBAC（成员身份和服务端授权完成验证前保持禁用）；Windows 读屏兼容性（NVDA，测试阶段）；Docker/INTEGRATION；真实 GGUF/LLM 与 GPU；麦克风/STT/TTS；摄像头/视觉；树莓派 GPIO/I2C/串口、传感器、电力与硬件 GPS；跨主机火种网络；可移动介质灾难恢复 |
