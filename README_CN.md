...
双门禁的条目仍可作为目录项检索，知识转任务接口也会拒绝执行；当前随包知识
仍待外部复核。
...
| Tier 1 | 短期生存（农业/化学/力学/天气/能源） | 12 |
| Tier 2 | 中期自足（农业/医疗/工程/社区） | 28 |
| Tier 3 | 长期社区（通信/法律/教育/文明档案） | 13 |
| SKF 知识包 | ZIP 格式标准化知识导入/导出，SHA256 校验 |

以上是每种语言的收录主题数，不是已审阅数。当前内置条目均待外部审阅；操作内容展示和知识转任务必须同时通过本地内容证据与具名风险审批。
`python scripts/knowledge_review_inventory.py --require-reviewed` 输出完整双语/hash 清单，门禁未通过时明确失败。
