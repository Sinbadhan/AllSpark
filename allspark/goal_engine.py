...
from allspark.models import Goal, Milestone, ResourceType, OperatingMode
...
class GoalEngine:
    ...
    def adjust_for_weather(self, weather_prediction: dict) -> list[dict]:
        adjustments = []
        forecast = weather_prediction.get("forecast", "unknown")
    ...
    def get_goal_summary(self) -> str:
        ...
        for g in sorted_goals:
            ...
            lines.append(
                f"{icon} [{g.priority}] {g.title}\n"
                f"   {g.rationale or g.description}\n"
                f"   进度: {ms_info} ({progress_pct}%){deadline_info}"
            )
            ...
            for ms in milestones:
                if ms.done and not ms.completed_at:
                    notifications.append({
                        ...
                        "message": f"里程碑达成：{ms.description}",
                        "priority": "medium",
                    })
    ...
    def sync_tasks_from_goal(self, goal_id: str) -> list:
        from allspark.models import Task
        ...
        for ms in milestones:
            ...
            task = Task(
                ...
                priority=priority,
                title=ms.description,
                description=f"[目标: {goal.title}] {ms.description}",
                status="pending",
                created_at=now,
                updated_at=now,
            )
    ...
    def on_task_completed(self, task_id: str):
        task = None
        for t in self.db.get_active_tasks():
            if t.id == task_id:
                task = t
                break
